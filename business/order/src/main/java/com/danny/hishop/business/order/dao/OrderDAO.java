package com.danny.hishop.business.order.dao;
...
public interface OrderDAO {
    ...
    @Insert("insert into t_order(orderNo,userName,receiverName,receiverMobileNo,receiverAddress,payType,status,deliverType,deliverTime,totalPrice,cutDownPrice,freight,actualPrice) " +
            "select #{orderNo},#{userName},#{receiverName},#{receiverMobileNo},#{receiverAddress},#{payType},#{status},#{deliverType},#{deliverTime},#{totalPrice},#{cutDownPrice},#{freight},#{actualPrice} " +
            "from dual where not exists " +
            "(select 1 from t_order where orderNo=#{orderNo})")
    @SelectKey(before = false, keyProperty = "id", resultType = Long.class, statementType = StatementType.STATEMENT, statement = "SELECT LAST_INSERT_ID() AS id")
    int insertOrderDOIdempotently(OrderDO orderDO);
